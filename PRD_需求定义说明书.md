...
* **FR-1.2 极简顶栏与 DSH 界面内嵌**：
  ...
  * **接近零 UI 开发成本**获得 Thinking 思维链折叠展收、Tool Calls 状态卡片、会话历史、工作区选择与权限审批（复用 DSH 视觉层；⚠️ 上下文胶囊、意图嗅探、草稿写入等**桥接逻辑仍需自写 DSH client 插件**，见详细设计 §0.3）。
...
* **FR-2.0 [“看左边”意图即刻全量快照] (Intent-Driven Full Snapshot)**：
  ...
  * ⚠️ 性能口径以详细设计 §1.2 为准（轻量页 p50 ≤300ms / 标准页 p95 ≤800ms / 含截图 p95 ≤1500ms，**字幕不计入该指标**）。
* **完全省去后台实时追踪**：不写“看左边”就是普通的纯本地任务，写了“看左边”就瞬间把左侧拉入战局，既保证读到最新页面，又彻底杜绝无效 Token 挥霍与电池消耗。
* **FR-2.1 [网页视频多维度读取] (Video & Media Reading)**：
  * **字幕提取（Transcript）**：针对包含视频的网页（如 YouTube、Bilibili、网课平台），**尽力提取**字幕与台词文本（best-effort），带时间戳喂给 Agent。⚠️ 覆盖矩阵见详细设计 §12.2：同源/带 CORS 的 `<track>` 可完整读取；播放器自绘字幕（YouTube/B站等）通常只能取到当前 cue 及其后内容；DRM 视频不可得且截图多为黑帧。
