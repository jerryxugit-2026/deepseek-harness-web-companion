...
**版本** v3.2 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| `POST /ag/ticket` | key + Origin（扩展） | 签发**一次性进入票据**（30s TTL、单次消费），供拼接 `enter` URL（§5.4 key 生命周期） |
| `GET /ag/enter?ticket=` | **导航：仅 ticket** | 校验并消费 ticket → 签发 `SameSite=None; Secure` cookie → `303 /` + `Referrer-Policy: no-referrer` |
...
|---|---|---|---|---|
| **导航**（iframe `src`、弹窗） | **不发送** | `cross-site` | **仅 ticket/key**（常量时间比较） | 若要求 Origin 会误杀全部合法导航 |
| **fetch/XHR**（微壳、SW） | `chrome-extension://<id>` | `cross-site` | **key + Origin 精确匹配** | |
| **WS 升级** `/ag/agent`（侧边栏文档） | `chrome-extension://<id>` | `cross-site` | **key + Origin 精确匹配** | |
| **fetch/WS** `/ag/client`（**DSH 页面内的 client 插件，同源**） | `http://127.0.0.1:<port>` | `same-origin` | **key + 有效 DSH 会话 cookie + Origin 必须等于本服务 authority**（**不得**要求 `chrome-extension://`） | v3.1 遗漏此形态，导致 §5.2 的 `request-pending`/`hello`/`ack` 恒 403（PiMoa BLOCKER-1） |

**key 生命周期（v3.2 明确，解决 PiMoa BLOCKER-2）**——两把钥匙，职责分离：

| 凭据 | 生成 | 存放 | 生命周期 | 用途 |
|---|---|---|---|---|
| **配对 key**（32 字节，长期） | `scripts/init-key.mjs` | `$DSH_HOME/dsh-web-companion.json`(0600) + `chrome.storage.local` | 长期；仅重装/轮换时更换 | 所有 `/ag/*` 的 fetch/WS 鉴权（微壳、SW、client 插件） |
...
**流程闭合**：自动拉起（§7.2）之后，微壳用**已存在的配对 key** 调 `/ag/ping` 与 `/ag/ticket`，因此不依赖"拉起时生成新 key"——配对 key 只在安装期生成一次，与被拉起的进程无关。**不采纳** UA/Referer 校验（可伪造 / 受 `Referrer-Policy` 影响，无安全价值）。
...
**权限最小化（v3.2 修正）**：默认仅 `sidePanel / storage / scripting / activeTab / tabs / alarms` + `host_permissions: http://127.0.0.1/*`；**`cookies`、`debugger`、`*://*/*` 全部改为 optional**——正式认证走服务端 `/ag/enter` 下发 `Set-Cookie`，`chrome.cookies.set` 只是 FINDINGS spike 与紧急兜底手段（PiMoa MAJOR：原先把 `cookies` 放进默认集与最小化主张自相矛盾）；**不声明 `content_security_policy`**；不申请 `<all_urls>`。
...
| 里程碑 | 内容 | 预算 | 退出条件（可测） |
|---|---|---|---|
| **M0a · 环境与能力证伪**（ADR-10 前置，**可与 M1 并行**） | 三个只读实验，不写产品代码：Q1（Strict 在 iframe 内 fetch 通/WS 不通的根因，3 cookie × 4 上下文 × 开关 host_permissions）、Q2（3P 屏蔽 12 次请求矩阵）、**Q3 提上来**（`createDraftImages ?? createDraftAttachments` 运行时存在性 + 契约路径复核 + `__ModuleLoader__` seed 内省） | ≤1 天 | 三份实验报告落盘 `docs/reviews/`；据此确定 §7.4 的 3P 兜底与 §6.3 的图片附件路径 |
