...
**版本** v3.12 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| **M1 · 骨架与认证**（协议单源 schema + codegen + 契约测试 **已完成，见 CHANGELOG v3.10**）；扩展 TS + Vite 工程；`/ag/ping`、`/ag/ticket`、`/ag/enter` + cookie 签发（固定向量 + ticket 单次消费测试）；iframe 嵌入与失败降级；配对脚本 `init-key.mjs`；`vite build` 体积报告 | 2.5 天 | E2E-1 通过；**L0 全绿**；体积报告产出（G6 可证伪）；插件安装走 `cordis.patch.yml` 绝对路径条目 |
