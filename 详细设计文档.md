...
**版本** v3.6 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| **F4** | **fetch/WS** `/ag/client`（**DSH 页面内的 client 插件，同源**） | `http://127.0.0.1:<port>` | `same-origin` | **key + 有效 DSH 会话 cookie + Origin 必须等于本服务 authority**（**不得**要求 `chrome-extension://`） | v3.1 遗漏此形态，导致 §5.2 的 `request-pending`/`hello`/`ack` 恒 403（PiMoa BLOCKER-1） |
