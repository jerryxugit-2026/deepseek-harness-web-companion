...
**版本** v3.3 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| 草稿读取/写入 | `const actx = ctx.sessions.scope(sessionId)`；`const shell = ctx.conversation.input.for(actx)`；`shell.setDraft(text)` / `shell.addImages(ids)` / `shell.submit()`；`setDraft` 是**唯一整段写入**入口 | 【源码·可复现】`@deepseek-ai/dsh@0.1.2-rc.1` **安装产物**：`…/@deepseek-ai/dsh-client-ui-conversation/lib/types/client/contract/input.d.ts:202-213`、`…/contract/slots.d.ts:299-318`。⚠️ PiMoa 亲验：源码检出 `/Volumes/Ex/ai_workspace/deepseek-harness/packages` 下**未命中**同路径（疑似 src/ 布局或分支差异）——本设计以安装产物为准，并在 **M0a 用探针复核** |
...
| **插入/撤销契约** | `setDraft` 是唯一整段写入入口，因此必须**记住插入前后的草稿快照与字符区间**：插入时保存 `{preDraft, insertedSpan:{start,end}, draftRev}`；用户点胶囊 ✕ 时——①若当前草稿仍等于插入后内容（`draftRev` 未变）→ **整段回退到 `preDraft`**；②若用户已继续编辑 → **只删除 `insertedSpan` 区间**并保留用户输入；③若 `draftRev` 已被 DSH 内部重写导致区间失效 → **不动草稿**，仅把胶囊标记为 `detached` 并提示"引用文本请手动删除"。**禁止**无条件整段覆盖（会吞掉用户输入） | 补 PiMoa MAJOR（v3.2 只写了插入，未写撤销） |
...
**授权与降级契约**：
1. 首次点「👀 看左边」（或首次意图触发）时，若尚无 `*://*/*` 授权 → 微壳弹出**一次**说明卡：「需要读取当前网页内容与截图；仅在本机处理」+ [授权] [仅本次用图标授权]；
2. 用户拒绝或未授权时，降级路径 = **要求扩展侧手势**：微壳提示「请点一下工具栏图标或按 ⌘⇧E 授权当前页」，随后走 `activeTab`；
3. **任何抓取失败都必须给出可诊断原因**（区分"未授权/页面受限/注入失败"），不得静默失败；
4. 该权限的授予状态由 `chrome.permissions.contains` 在每次抓取前检查，并在微壳顶栏以图标提示当前模式（「已授权自动抓取」/「需手势」）。
...
| **B** | **DSH 暴露可读的 composer 草稿状态，且所指版本的契约路径/命名稳定** | 「看左边」只能退化为"顶栏按钮 + 快捷键"（仍可用，但失去自然语言触发） | Q3（M0a）+ Q4（M2）实验 + 读 `SessionInput`/`useInput` 状态机；**并核对契约路径在所指版本中确实存在**（PiMoa 亲验未命中，故假设 B 当前状态为"未证实"而非"已知"） | 保留按钮/快捷键触发；胶囊改为面板顶栏条带（不依赖 DSH 插槽） |
