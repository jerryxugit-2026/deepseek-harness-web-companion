...
**版本** v3.19 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| 通道 | 持有者 | 心跳 | 断连处理 | 重连后 |
|---|---|---|---|---|
| `/ag/agent`（工具桥） | 侧边栏文档 | 20s `ping` / 30s 判离线 | 插件**立即**以 `E_EXT_OFFLINE` 拒绝在途请求（不等超时） | 指数退避 1s→2s→…→30s；重连后发 `hello` |
