...
**版本** v3.4 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| **`*://*/*`** | **`optional_host_permissions`，首次使用引导授予** | **「看左边」是主路径**：它由 iframe 内的 client 插件发起（F4 → 桥接插件 → SW），**全程没有扩展侧用户手势**，`activeTab` 在这种情形下不生效 → `executeScript` 与 `captureVisibleTab` 会**同时失权**。因此抓取能力必须靠**显式授予的 host 权限**支撑 |
...
**M0a 必测**：①在微壳按钮点击回调里调 `permissions.request` 是否成功（手势是否被认可）；②**无手势**调用 `permissions.request` 的 `lastError` 原文（作为"意图路径无法自行授权"的证据）；③授权后由 F4 意图路径触发的 `executeScript` 与 `captureVisibleTab` 是否均成功。
...
| **A′** | **"首启按钮授权 + 无手势意图抓取"能共存**（当前判断为**大概率互斥**，故列为最脆弱项） | 「看左边」的自然语言入口失效 → 退回"微壳按钮点击抓取"（体验降级，主干不受影响） | M0a 权限实验：微壳按钮内 `permissions.request` 是否成功；无手势调用的 `lastError` 原文；授权后 F4 意图路径的 `executeScript`/`captureVisibleTab` 是否成功 | 方案②降级（按钮确认后再抓）作为主路径；G2 改为"按钮触发"验收 |
