...
**版本** v3.25 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| 开关 | `intentEnabled`（默认 true）、`intentCaptureMode`（默认 `page`）、`attachSessionMode`（默认 `new`）、`retentionHours`（默认 24，`0` 关闭） | 设计 |
