...
**版本** v3.23 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| `debugger` | optional | M3 增强（可信输入 + AX 树）；带不可消除横幅 |
...
| **M3 · 反向控制** | `WS /ag/agent` + `ops/*`；`browser_*` 工具注册与错误映射；**debugger opt-in 增强**（可信输入 + AX 读取，`requiredVersion:"1.3"`）；权限接线 | 3 天（v3.0 为 2 天——按 PiMoa 意见上调，因调试器路径需额外接线与横幅 UX） | E2E-7 通过；**点击/输入成功率目标：普通 HTML 页 100%、主流 SPA ≥80%，且失败必须可诊断（返回 `E_TARGET` + 候选元素）**——不承诺"可靠"；写操作默认不可用 + 开启后受审批 |
