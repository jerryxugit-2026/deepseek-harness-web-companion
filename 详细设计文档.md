...
**版本** v3.5 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| 草稿读取/写入 | `const actx = ctx.sessions.scope(sessionId)`；`const shell = ctx.conversation.input.for(actx)`；`shell.setDraft(text, editRange?)` / `shell.addImages(ids)` / `shell.submit()` | 【源码·已亲验】**源码检出** `deepseek-harness/packages/client/ui-conversation/src/client/input/contract.ts`：`setDraft`/`addImages`/`submit` 于 `:35/:37/:46`；**真实签名 `setDraft(text, editRange?)` 于 `:104`**，配套 `EditRange` 类型 `:147`、`draft-changed` 事件 `:253`。同一符号在**安装产物** `…/@deepseek-ai/dsh-client-ui-conversation/lib/types/client/contract/input.d.ts:202-213`（`InputActions`）另有一份声明。⚠️ v3.1–v3.3 的"`setDraft` 是唯一整段写入入口"**已被证伪**（存在 `editRange` 参数），据此写的三分支撤销契约按下方"插入/撤销契约"简化 |
...
| **插入/撤销契约（v3.4 按亲验签名重写）** | 因 `setDraft(text, editRange?)` 支持区间写入，插入与撤销都用**区间**而非整段覆盖：插入时记录 `{preText, range: EditRange, draftRev}`（并订阅 `draft-changed` 维护 `draftRev`）；用户点 ✕ 时——①`draftRev` 未变 → `setDraft(preText, 覆盖 range 的等价区间)` 回退；②用户已继续编辑（`draftRev` 变化）→ **只替换我们的区间**，保留用户输入；③区间已失效 → **不动草稿**，仅把胶囊标记 `detached` 并提示手动删除。**禁止**无条件整段覆盖。⚠️ `EditRange` 的确切语义（是否支持"替换/仅插入"）与 `draft-changed` 的触发时机需在 **M0a 复核**（Q4 已改写） | 补 PiMoa MAJOR；v3.3 的版本基于伪约束，已简化 |
