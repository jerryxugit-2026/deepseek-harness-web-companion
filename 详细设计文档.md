...
**版本** v3.7 · **日期** 2026-09-11 · **状态** 设计定稿（H4 已决：默认启用自动拉起；改选 ② 时按 §0.2 的一组三处同步切换）
...
| Q8 | 「空闲 WS 是否保活 SW/文档」的实测基线：§11.1 称 116+ 仅**流量**重置计时，但侧边栏文档被冻结/隐藏时行为未实测 | 影响 §6.2 把 `/ag/agent` 放在侧边栏文档的决定是否可靠 | 侧边栏保持 WS 空闲 5 分钟，观察连接是否被断、文档是否被丢弃；对照组：每 20s 主动 `ping` | M0a |
