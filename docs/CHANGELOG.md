...
这一版补上两件收尾：计划不再谎报动作，且这条保护有会咬人的测试守着。

### 1. 计划里 ①② 两行按"原地 / 异地"分别如实说明

原来无论哪种情况都打印「① 创建安装目录 <路径>」，原地安装时这句话是假的（目录已存在、也不会复制源码）。
现在原地安装打印：
...
异地安装仍打印「① 创建安装目录 <路径>」「② 复制源码 N 项」。
`inPlace` 的定义从第 2 步前移到"计划"之前 —— 否则 dry-run 看不到这个分支，也就没法在**不动任何文件**的
前提下验证它。

### 2. ★ 回归门禁（`/Users/mac/ai_tools/dsh project/网页插件/tests/unit/install-behavior.test.mjs` 第 4 节）

9 条新断言，其中关键是**双向**的（防止硬编码骗过测试）：

- 不给 `--install-dir` ⇒ 缺省安装目录 == 本包目录，计划说「沿用本目录」「复制源码 跳过」；
- 显式给 `--install-dir` ⇒ **不得**出现「沿用本目录」，且计划为「创建安装目录」+「复制源码 N 项」。

**验真**：把 `defaultInstallDirPath` 退回旧写法 `resolve(defaultInstallDir(homeDir))`，这 3 条立刻变红
（`exit=1`）；恢复后 25 条断言全绿（`exit=0`）。

### 3. 文档说反了的地方

`/Users/mac/ai_tools/dsh project/网页插件/README.md` 中英文两段原本写着「**不要**装进你解压出来的那个
文件夹」（那是旧缺省值下的建议），与 v3.47.2 的新行为正好相反 —— 已改成「**它就装在你解压出来的那个
文件夹里 —— 这是缺省值，而且是安全的**」。同时把 README 里 4 处 `3.47.1` 的示例路径/版本改成 `3.47.3`。

### 4. 版本号 3.47.3（5 处）

由 `tests/unit/version-consistency.test.mjs` 守住。

---

## v3.47.2 — 2026-09-13（缺省安装目录改成"你解压的那个文件夹"）

**触发**：用户 2026-09-13 明确要求：「我解压在哪个目录, 程序就应该缺省安装在哪个目录, 不要你自己设置一个目录」。

### 1. 缺省安装目录 = 你解压出来的这个文件夹

原来是 `~/.dsh/plugins/dsh-web-companion`。现在缺省就是**运行安装器的那个包目录**（`bootstrap/` 的上一级）；
`--install-dir` 仍然优先；交互时也会问你一次，默认值就是这个。

### 2. ★ 安全前提：原地安装必须整段跳过"先删再拷"

第 2 步（复制源码）原本是"先 `rm -rf` 再把源码拷过去"—— 当**安装目录 == 源码目录**时，
它会先删掉源码、再试图从已删掉的路径复制，**把用户解压出来的包毁掉**。
改成默认安装目录之后这条会变成常规路径，所以原地安装时**整段跳过复制**（源码本来就在位）。

### 3. 文档同步

`docs/13-安装部署.md`、`docs/13-installation.md`、`--help` 里的"默认安装目录"说明一起改掉。
